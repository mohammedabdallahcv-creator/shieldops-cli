...
Available commands:
  ...
  /k8s-scan         Scan Kubernetes YAML (requires API key)
  /scan-image       Scan Docker image (local, requires Trivy)
...
def run(theme: str = "dark") -> None:
    ...
    while running:
        ...
        cmd, output = process_command(user_input, last_output, session, prompt_text)
        if output:
            last_output = output
...
def process_command(
    user_input: str, last_output: str,
    session: PromptSession | None = None,
    prompt_style: FormattedText | None = None,
) -> tuple[str, str]:
    ...
    try:
        output = handler(args, last_output, session, prompt_style)
...
def _tui_input(session: PromptSession, prompt_text: str, style: FormattedText | None = None) -> str:
    """Get input using the TUI prompt session, falling back to input()."""
    if session is not None and style is not None:
        try:
            return session.prompt(
                FormattedText([("dim", f"{prompt_text} ")])
            ).strip()
        except (EOFError, KeyboardInterrupt):
            return ""
...
def cmd_autofix(args: str | None, last_output: str,
                session: PromptSession | None = None,
                style: FormattedText | None = None) -> str:
    ...
    if not client.api_key:
        console.print("[red]Autofix requires an API key. Run /login first.[/red]")
        return ""
...
def cmd_sbom(args: str | None, last_output: str,
             session: PromptSession | None = None,
             style: FormattedText | None = None) -> str:
    """Generate SBOM (requires API key)."""
    if not args:
        file_path = _tui_input(session, "Path to file (Dockerfile, requirements.txt, ...): ", style)
        if not file_path:
            return ""
        args = file_path
    ...
    client = ShieldOpsClient()
    if not client.api_key:
        console.print("[red]SBOM requires an API key. Run /login first.[/red]")
        return ""
...
def cmd_compose_scan(args: str | None, last_output: str,
                     session: PromptSession | None = None,
                     style: FormattedText | None = None) -> str:
    """Scan docker-compose.yml (requires API key)."""
    if not args:
        file_path = _tui_input(session, "Path to docker-compose.yml: ", style)
        if not file_path:
            return ""
        args = file_path
    ...
    client = ShieldOpsClient()
    if not client.api_key:
        console.print("[red]Compose scan requires an API key. Run /login first.[/red]")
        return ""
...
def cmd_compose_generate(args: str | None, last_output: str,
                         session: PromptSession | None = None,
                         style: FormattedText | None = None) -> str:
    ...
    client = ShieldOpsClient()
    if not client.api_key:
        console.print("[red]Compose generation requires an API key. Run /login first.[/red]")
        return ""
    ...
    if generated:
        out_path = _tui_input(session, "Save as (default: docker-compose.yml): ", style)
        if out_path:
            Path(out_path).write_text(generated, encoding="utf-8")
            console.print(f"[green]Compose file saved to {out_path}[/green]")
        else:
            Path("docker-compose.yml").write_text(generated, encoding="utf-8")
            console.print("[green]Compose file saved to docker-compose.yml[/green]")
    else:
        from shieldops_cli.formatters.table import render_table
...
def cmd_k8s_scan(args: str | None, last_output: str,
                 session: PromptSession | None = None,
                 style: FormattedText | None = None) -> str:
    """Scan Kubernetes manifest (requires API key)."""
    if not args:
        file_path = _tui_input(session, "Path to Kubernetes YAML: ", style)
        if not file_path:
            return ""
        args = file_path
    ...
    client = ShieldOpsClient()
    if not client.api_key:
        console.print("[red]K8s scan requires an API key. Run /login first.[/red]")
        return ""
...
def cmd_scan_image(args: str | None, last_output: str,
                   session: PromptSession | None = None,
                   style: FormattedText | None = None) -> str:
    """Scan a Docker image locally using Trivy."""
    if not args:
        image_name = _tui_input(session, "Image name (e.g. nginx:latest): ", style)
        if not image_name:
            return ""
        args = image_name
...
def cmd_login(args: str | None, last_output: str,
              session: PromptSession | None = None,
              style: FormattedText | None = None) -> str:
    """Login with API key."""
    ...
    if not key:
        key = _tui_input(session, "API key: ", style)
        if not key:
            return ""
...
def cmd_logout(args: str | None, last_output: str,
               session: PromptSession | None = None,
               style: FormattedText | None = None) -> str:
...
def cmd_whoami(args: str | None, last_output: str,
               session: PromptSession | None = None,
               style: FormattedText | None = None) -> str:
    ...
    if not api_key:
        ...
        console.print("Run /login for cloud features")
        return "Free mode"
...
def cmd_config(args: str | None, last_output: str,
               session: PromptSession | None = None,
               style: FormattedText | None = None) -> str:
...
def cmd_save(args: str | None, last_output: str,
             session: PromptSession | None = None,
             style: FormattedText | None = None) -> str:
    ...
    if not args:
        ...
        save_path = _tui_input(session, f"Save as (default: shieldops_report_{timestamp}.txt): ", style)
        if not save_path:
            save_path = f"shieldops_report_{timestamp}.txt"
        args = save_path
...
def cmd_help(args: str | None, last_output: str,
             session: PromptSession | None = None,
             style: FormattedText | None = None) -> str:
    """Show help text."""
    print(HELP_TEXT)
...
def cmd_clear(args: str | None, last_output: str,
              session: PromptSession | None = None,
              style: FormattedText | None = None) -> str:
    """Clear the screen."""
    clear()
...
def cmd_exit(args: str | None, last_output: str,
             session: PromptSession | None = None,
             style: FormattedText | None = None) -> str:
    """Exit the TUI."""
    print("Session closed.")
